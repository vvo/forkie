...
function worker(title, fns) {
  ...
  var graceful = {
    start: function() {
      fns.start(function() {
        if (process.send) {
          process.send({graceful: {status: 'started'}});
        }
    ...
    stop: function() {
      ...
      fns.stop(function() {
        if (process.send) {
          process.send({graceful: {status: 'stopped'}});
  ...
  var exitSignalHandler = partial(handleMasterMessage, {graceful: {action: 'stop'}});
  ...
  function init() {
    ...
    if (process.on && process.send) {
      ...
      process.once('disconnect', masterDied);
  ...
  function subscribe(signal) {
    process.once(signal, partial(exitSignalHandler, signal));
  }
  ...
  function masterDied() {
    ...
    process.nextTick(partial(process.exit, 1));
  }
